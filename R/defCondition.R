...
#' # Add columns to dt
#'
#' defC <- defCondition(condition = "x == 1", formula = "5 + 2*y", variance = 1,dist = "normal")
#' defC <- defCondition(defC, condition = "x <= 5 & x >= 2", formula = "3 - 2*y", variance = 1, dist="normal")
#' defC <- defCondition(defC, condition = "x >= 6", formula = 1, variance = 1, dist="normal")
#'
#' defC
#'
