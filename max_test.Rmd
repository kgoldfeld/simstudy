...
typeof(assertIntegerOrFactor(var1 = test))
```


```{r}
argu <- data.table::data.table(defData(varname = "rx", formula = 5, dist = "normal"))

newColumn <- .genAssign(
      dtName = copy(NULL),
      grpName = argu$varname,
      strata = argu$variance,
      ratio = argu$formula,
      balanced = argu$link
    )
newColumn
