...
For a cateogrical variable with <span class="inlinecode">$k$</span> categories, the formula is a string of  probabilities that sum to 1: "<span class="inlinecode">$p_1, p_2, ..., p_k$</span>". <span class="inlinecode">$p_1$</span> is the probability of the random variable falling category 1, <span class="inlinecode">$p_2$</span> is the probablity of category 2, etc. The probabilities cannot be specified as functions of other variables previously defined. In the example, `xCat` has three possibilities with probabilites 0.3, 0.2, and 0.5, respectively.\
...
After the data set definitions have been created, a new data set with <span class="inlinecode">$n$</span> observations can be created with a call to function **`genData`**. In this example, 1,000 observations are generated using the data set defitions in **`def`**, and then stored in the object **`dt`**:
